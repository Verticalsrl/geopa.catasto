...
# Ciclo sui dati FAB, TER, SOG, e TIT per recuperare la radice comune del nome dei file, ed avviare lo script, scritto da me, "fab2db.py".
...
def main(cxf_path, format_dst, db_path, dirLog, schema='public'):
    ...
    #class_table_path_raw = "C:\\Python27\\Lib\\site-packages\\bgeo.catasto-0.1-py2.7.egg\\bgeo\\catasto"
    ...
    logname = "%s\\cat2db-%s" % (dirLog, start_date)
    ...
    try:
        ...
        if not uniqueList == []:
            ...
            #METODO 1: Importo le classi di tavole gia' definite in db.py, pero' non posso specificare lo schema!
            '''
            class_table_path = os.path.abspath(class_table_path_raw)
            if not class_table_path in sys.path:
                sys.path.insert(1, class_table_path)
            #Dallo script "db.py" mi devo importare tutte le classi di tavole definite:
            from db import Ut_Comuni, Indirizzi, Identificativi, Unita_imm, Riserve
            from db import Terreni, Porzioni, CT_Riserve, Deduzioni
            from db import Soggetto, Titolarita
            #Riporto le tabelle in una lista:
            tables = []
            tables.append(Ut_Comuni.__table__)
            tables.append(Terreni.__table__) #e cosi' via...
            #Ciclo nella lista creata per troncare le tavole:
            for table in tables:
                engine.execute(table.delete())
            '''
            #METODO 2: Specifico direttamente le tabelle creando una transazione:
            #Riporto le tabelle in una lista:
            tables = ['cf_immobili', 'cf_identificativi', 'cf_indirizzi', 'cf_riserve', 'cf_utilita_comuni', 'ct_terreni', 'ct_porzioni', 'ct_riserve', 'ct_deduzioni', 'cu_soggetti', 'cu_titolarita']
        ...
        # Bisognerebbe TRONCARE le tabelle degli immobili prima di caricare i dati nuovi. Come esser sicuri che ci siano i file corretti in cartella in modo da non svuotare la tabella per niente??
    finally:
        if connection:
            connection.close()
    ...
    try:
        ...
        if not infile:
            ...
            query_drop = "DROP TABLE IF EXISTS public.cat_tit_sog_fab;DROP TABLE IF EXISTS public.cat_tit_sog_ter;"
            connection_create.execute(query_drop)
            trans_drop.commit()
            trans_create = connection_create.begin()
            query_create = "CREATE TABLE public.cat_tit_sog_fab AS (SELECT * FROM schema.v_tit_sog_fab);COMMENT ON TABLE public.cat_tit_sog_fab IS 'tavola creata da codice a seguito di caricamento da file con i dati dalla vista schema.v_tit_sog_fab';ALTER TABLE public.cat_tit_sog_fab ADD COLUMN gid serial;ALTER TABLE public.cat_tit_sog_fab ADD PRIMARY KEY (gid);CREATE TABLE public.cat_tit_sog_ter AS (SELECT * FROM schema.v_tit_sog_ter);COMMENT ON TABLE public.cat_tit_sog_ter IS 'tavola creata da codice a seguito di caricamento da file con i dati dalla vista schema.v_tit_sog_ter';ALTER TABLE public.cat_tit_sog_ter ADD COLUMN gid serial;ALTER TABLE public.cat_tit_sog_ter ADD PRIMARY KEY (gid);"
        ...
        print "Fine operazione completa: %s" % end_time
