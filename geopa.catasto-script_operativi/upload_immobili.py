# In questo script entro in una directory che contiene i file FAB, TER, TIT e SOG compattati in file .zip, dunque scompatto il file compresso ed inizio ad elaborare le informazioni caricandole su un DB.

#Viene richiamato con:
#python.exe C:\\workspace\\DocWEB\\orbassano\\script\upload_immobili.py "C:\\workspace\\DocWEB\\orbassano\\File_upload\\immobili" "C:\\workspace\\DocWEB\\orbassano\\File_upload\\processati\\immobili" "C:\\workspace\\DocWEB\\orbassano\\script" -d "postgres://postgres:root@localhost:5433/geo_test" public
...
file_processed = 'FAB-TER-TIT-SOG'
logfile = "%s\\\cat2db-%s" % (dirLog, start_date)
...
############################### Carico i file TER, FAB, TIT e SOG sul DB: #################################

ciclo_cat2db.main(my_dir, format_dst, db_path, dirLog, db_schema)
